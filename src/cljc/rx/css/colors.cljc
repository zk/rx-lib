...
(defn parse-google-colors [copy-paste-swatch]
  (->> (str/split
         copy-paste-swatch
         #"\n")
       (partition 2)
...
(defn lightness [s]
  (double (:lightness (gc/hex->hsl s))))

(def google-colors-in-order
  [:red [:50 "#FFEBEE" :100 "#FFCDD2" :200 "#EF9A9A" :300 "#E57373" :400 "#EF5350" :500 "#F44336" :600 "#E53935" :700 "#D32F2F" :800 "#C62828" :900 "#B71C1C" :A100 "#FF8A80" :A200 "#FF5252" :A400 "#FF1744" :A700 "#D50000"]
   ...
   :dark-orange [:50 "#FBE9E7" :100 "#FFCCBC" :200 "#FFAB91" :300 "#FF8A65" :400 "#FF7043" :500 "#FF5722" :600 "#F4511E" :700 "#E64A19" :800 "#D84315" :900 "#BF360C" :A100 "#FF9E80" :A200 "#FF6E40" :A400 "#FF3D00" :A700 "#DD2C00"]

   :brown [:50 "#EFEBE9" :100 "#D7CCC8" :200 "#BCAAA4" :300 "#A1887F" :400 "#8D6E63" :500 "#795548" :600 "#6D4C41" :700 "#5D4037" :800 "#4E342E" :900 "#3E2723"]

   :blue-grey [:25 "#f9fbfc" :50 "#ECEFF1" :100 "#CFD8DC" :200 "#B0BEC5" :300 "#90A4AE" :400 "#78909C" :500 "#607D8B" :600 "#546E7A" :700 "#455A64" :800 "#37474F" :900 "#263238"]])

(def google-colors-map
  (->> google-colors-in-order
       (partition 2)
       (map (fn [[k v]]
              [k (->> v
                      (partition 2)
...
(defn red [k]
  (or (get
       (:red google-colors-map)
       k)
      'red))

(defn pnk [k]
  (or (get
       (:pink google-colors-map)
        k)
      'red))

(defn prp [k]
  (or (get
       (:purple google-colors-map)
        k)
      'red))

(defn dprp [k]
  (or (get (:deep-purple google-colors-map)
        k)
      'red))

(defn ind [k]
  (or (get (:indigo google-colors-map)
        k)
      'red))

(defn blu [k]
  (or (get (:blue google-colors-map)
        k)
      'red))

(defn lb [k]
  (or (get (:light-blue google-colors-map)
        k)
      'red))

(defn cyn [k]
  (or (get (:cyan google-colors-map)
        k)
      'red))

(defn teal [k]
  (or (get (:teal google-colors-map)
        k)
      'red))

(defn gr [k]
  (or
    (get (:green google-colors-map)
      k)
    'red))

(defn lgr [k]
  (or (get (:light-green google-colors-map)
        k)
      'red))

(defn lime [k]
  (or (get (:lime google-colors-map)
        k)
      'red))

(defn yl [k]
  (or (get (:yellow google-colors-map)
        k)
      'red))

(defn amb [k]
  (or (get (:amber google-colors-map)
        k)
      'red))

(defn org [k]
  (or (get (:orange google-colors-map)
        k)
      'red))

(defn dorg [k]
  (or (get (:dark-orange google-colors-map)
        k)
      'red))

(defn brn [k]
  (or (get (:brown google-colors-map)
        k)
      'red))

(defn blgr [k]
  (or (get (:blue-grey google-colors-map)
        k)
      'red))
