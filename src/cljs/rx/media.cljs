...
(s/def ::ios-local-identifier ::specs/non-empty-string)
(s/def ::s3-bucket ::specs/non-empty-string)
(s/def ::s3-key ::specs/non-empty-string)
(s/def ::mime-type ::specs/non-empty-string)
(s/def ::uri ::specs/non-empty-string)
(s/def ::width nat-int?)
(s/def ::height nat-int?)
(s/def ::orientation #{1 2 3 4 5 6 7 8})

(s/def ::media
  (s/keys
    :opt [::ios-local-identifier
          ::s3-bucket
          ::s3-key
          ::mime-type
          ::uri
...
(s/def ::original ::media)
(s/def ::thumbs (s/coll-of ::media :into []))
(s/def ::remote ::media)

(defn remote-uri [media]
  (or (-> media ::remote ::uri)
      (::uri media)))

(defn format-orientation [o]
  (cond
    (number? o) o
    (string? o) (ks/parse-long o)
    :else nil))
...
(defn rn-image-crop-picker-item->media
  ...
  (let [mime-type (if (and filename
        ...
        orientation (-> exif
                        (get "Orientation")
                        format-orientation)]
    (->> {::id (or localIdentifier path)
          ::ios-local-identifier localIdentifier
          ::width width
          ::height height
          ::size-bytes size
          ::created-ts (* (ks/parse-long creationDate) 1000)
          ::updated-ts (* (ks/parse-long modificationDate) 1000)
          ::file-path path
          ::mime-type mime-type
