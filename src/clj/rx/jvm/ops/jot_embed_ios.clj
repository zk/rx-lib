...
(defn start-figwheel [{:keys [main-namespace
  ...
  (let [root-path (dev-path opts)
    ...
    (try
      #_(fapi/stop safe-main-namespace)
      (catch Exception e
...
(comment
  ...
  (start-figwheel
    {:output-path "./vee3/jot-react-native-embed"
  ...
  #_(fapi/stop "rx-browser-jot-example-embed")
  ...
  (let [main-namespace 'rx.browser.jot.example-embed
    ...
    #_{:figwheel-options
       {:repl true
        ...
        :server-port 44200
        :server-logfile "target/dev/server/figwheel_server.log"}
       :build-ids ["ios"]

       :all-builds
       [{:id "ios"
         :source-paths ["src/cljs-rn"
                        "src/cljs"
                        "src/cljc"
                        "src/clj"
                        "checkouts/nsfw/src/cljc"
         ...
         :compiler {:output-to "target/rn-dev/ios/index.js"
                    :main "rx.rn.test-entry"
                    ;;:output-dir "target/rn-dev/ios"
                    :source-map true
                    :optimizations :none
                    :target :nodejs
    ...
    #_(fapi/start
        {:watch-dirs ["src/cljs-rn"
         ...
         :mode :repl
         :open-url false
         :ring-server-options {:port 44200}
         :client-log-level :finest
         :validate-config true
         :rebel-readline false
         :figwheel-core false}
        {:id "ios"
         :options
         {:output-to "target/rn-dev/ios/index.js"
          :output-dir "target/rn-dev/ios"
          :optimizations :none
          :source-map true
          :main "rx.rn.test-entry"
