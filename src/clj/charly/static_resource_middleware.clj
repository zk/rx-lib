(ns charly.static-resource-middleware
   ...
   (let [options (merge {:root (str root-path)
     ...
     (if (#{:get :head} (:request-method request))
       (let [path (subs (codec/url-decode (request/path-info request)) 1)
         ...
         (if exact-match-resp
           exact-match-resp
           (let [path (str path ".html")
                 html-match-resp (-> (response/file-response path options)
                                     (head/head-response request))]
             (when html-match-resp
               (ks/deep-merge
                 html-match-resp
